...
static void microphone_send_thread() {
  if (!feature_microphone || microphone_socket < 0) return;
...
static void audio_receive_thread() {
  if (!feature_audio || audio_socket < 0) return;
  ...
  while (running && audio_playback.running) {
    if (recv(audio_socket, &type, sizeof(type), 0) != sizeof(type)) break;
    ...
    AudioDataHeader header;
    if (recv(audio_socket, &header, sizeof(header), 0) != sizeof(header)) break;
...
int main(int argc, char** argv) {
  ...
  if (feature_input) {
    ...
    if (input_socket < 0) {
      ...
      if (!feature_video && !feature_audio && !feature_microphone) return 1;
      feature_input = false;
    } else {
      sockaddr_in addr{};
      addr.sin_family = AF_INET;
  ...
  // Close sockets
  if (frame_socket >= 0) close(frame_socket);
  if (input_socket >= 0) close(input_socket);
  if (audio_socket >= 0) close(audio_socket);
  if (config_socket >= 0) close(config_socket);
