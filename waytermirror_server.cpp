...
static std::string detect_compositor(const std::string& override = "auto") {
  ...
  if (compositor) {
    ...
    if (comp.find("hypr") != std::string::npos) return "hyprland";
    if (comp.find("sway") != std::string::npos) return "sway";
    if (comp.find("river") != std::string::npos) return "river";
    if (comp.find("kde") != std::string::npos || comp.find("plasma") != std::string::npos) return "kde";
    if (comp.find("gnome") != std::string::npos) return "gnome";
  }
...
struct Capture {
  ...
  // ADD THIS:
  std::vector<uint8_t> stable_copy;  // Stable copy for rendering
...
static int detect_focused_output_hyprland() {
  ...
  if (doc.HasParseError() || !doc.IsObject()) return -1;
  if (!doc.HasMember("monitor") || !doc["monitor"].IsInt64()) return -1;
...
static int detect_focused_output_sway() {
  ...
  rapidjson::Document doc;
  doc.Parse(output.c_str());
  if (doc.HasParseError() || !doc.IsArray()) return -1;
...
static void update_focus_tracking() {
  if (!focus_tracker.follow_focus.load()) return;
...
static void frame_buffer(void* data, zwlr_screencopy_frame_v1* frame,
  ...
  cap->data = mmap(nullptr, cap->size, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
  ...
  cap->buffer = wl_shm_pool_create_buffer(pool, 0, width, height, stride, format);
...
static void frame_ready_cb(void* data, zwlr_screencopy_frame_v1* frame, 
  ...
  // Copy the frame data to stable buffer
  Capture* cap = ctx->capture;
  cap->stable_copy.resize(cap->size);
  memcpy(cap->stable_copy.data(), cap->data, cap->size);
...
static void toplevel_handle_closed(void* data, zwlr_foreign_toplevel_handle_v1* handle) {
  ...
  auto it = std::remove_if(focus_tracker.toplevels.begin(), focus_tracker.toplevels.end(),
    [handle](const ToplevelInfo& tl) { return tl.handle == handle; });
...
static void handle_key_event(const KeyEvent& evt) {
  ...
  if (server_shift_pressed) mods_depressed |= (1 << 0);   // Shift
  if (server_ctrl_pressed) mods_depressed |= (1 << 2);    // Control
  if (server_alt_pressed) mods_depressed |= (1 << 3);     // Alt/Mod1
  if (server_super_pressed) mods_depressed |= (1 << 6);   // Super/Win/Mod4
  if (server_altgr_pressed) mods_depressed |= (1 << 7);   // AltGr/Mod5
  
  // Locked modifiers (toggle keys)
  if (server_capslock_pressed) mods_locked |= (1 << 1);   // CapsLock
  if (server_numlock_pressed) mods_locked |= (1 << 4);    // NumLock
...
static uint8_t calculate_braille_pattern(
  ...
  // HIGH DETAIL (>= 90): Otsu on effective luminance (luma + saturation bonus)
  if (detail_level >= 90) {
    // Calculate effective luminance: luminance + saturation bonus for colored pixels
    double eff_luma[8];
    for (int i = 0; i < 8; i++) {
      uint8_t r = cell.colors[i][0];
      uint8_t g = cell.colors[i][1];
      uint8_t b = cell.colors[i][2];
      ...
      // Effective luminance: luma + saturation bonus
      // Saturated reds/greens/blues on dark backgrounds still show up
  ...
  uint8_t pattern = 0;
  for (int dot = 0; dot < 8; dot++) {
    double luma = cell.lumas[dot];
    double diff = luma - threshold;
    
    bool activate = false;
    if (detail_level < 50) {
      activate = (diff > 6.0) || (diff > -6.0 && diff > 0);
    } else {
      // Crisp threshold for detail
      activate = (diff >= 0);
    }
...
static void calculate_braille_colors(
  ...
  // Simple direct averaging - works best with k-means pattern
  ...
  if (lit_dots.empty()) {
    // All dots unlit - use average as both
    fg_r = unlit_r / 8; fg_g = unlit_g / 8; fg_b = unlit_b / 8;
    bg_r = fg_r; bg_g = fg_g; bg_b = fg_b;
  } else if (unlit_dots.empty()) {
    // All dots lit - use average as both
    fg_r = lit_r / 8; fg_g = lit_g / 8; fg_b = lit_b / 8;
    bg_r = fg_r; bg_g = fg_g; bg_b = fg_b;
  } else {
  ...
  // For detail >= 70, pattern already determined fg=bright, no inversion needed
  // For lower detail, check if inversion helps
  if (detail_level < 70) {
...
static std::string render_braille(
  ...
  if (!frame_data || frame_width == 0 || frame_height == 0) return "";
  
  std::ostringstream out;
  out.precision(0);
  out << std::fixed;
  
  // Calculate scaled dimensions (2x4 pixels per cell)
  int w_scaled, h_scaled;
  if (keep_aspect_ratio) {
    double src_aspect = (double)frame_width / frame_height;
    ...
    w_scaled = (int)(frame_width * scale_factor);
    h_scaled = (int)(frame_height * scale_factor);
  }
  ...
  for (int cy = 0; cy < cells_y; cy++) {
    for (int cx = 0; cx < cells_x; cx++) {
      // Analyze cell
      BrailleCell cell = analyze_braille_cell(
        frame_data, frame_width, frame_height, frame_stride,
        cx, cy, w_scaled, h_scaled, detail_level
      );
      
      // Regional color analysis
      RegionalColorAnalysis regional = analyze_regional_colors(
        frame_data, frame_width, frame_height, frame_stride,
        cx, cy, w_scaled, h_scaled, cells_x, cells_y, detail_level
      );
      
      // Calculate pattern
      uint8_t pattern = calculate_braille_pattern(cell, detail_level, threshold_steps, regional);
      
      // Calculate colors
      uint8_t fg_r, fg_g, fg_b, bg_r, bg_g, bg_b;
      calculate_braille_colors(cell, pattern, fg_r, fg_g, fg_b, 
...
static std::string render_hybrid(
  ...
  for (int cy = 0; cy < cells_y; cy++) {
    for (int cx = 0; cx < cells_x; cx++) {
      ...
      // CRITICAL: Use has_edge boolean - if edge exists AT ALL, use braille
      ...
      if (use_braille) {
        ...
        // Low-detail half-block rendering for flat areas
        uint32_t top_r = 0, top_g = 0, top_b = 0;
        uint32_t bot_r = 0, bot_g = 0, bot_b = 0;
        
        // Top half (dots 0,1,3,4) - rows 0-1
        for (int i : {0, 1, 3, 4}) {
        ...
        // Bottom half (dots 2,5,6,7) - rows 2-3
        for (int i : {2, 5, 6, 7}) {
...
static std::string render_blocks(
  ...
  for (int by = 0; by < blocks_y; by++) {
    for (int bx = 0; bx < blocks_x; bx++) {
      // Calculate source coordinates for top and bottom half
      int src_x = (bx * frame_width) / w_scaled;
      int src_y_top = ((by * 2) * frame_height) / h_scaled;
      int src_y_bot = ((by * 2 + 1) * frame_height) / h_scaled;
      ...
      for (int ky = 0; ky < kernel_size; ky++) {
        for (int kx = 0; kx < kernel_size; kx++) {
          int px = std::clamp(src_x + kx, 0, (int)frame_width - 1);
          ...
          // Top half
          int py_top = std::clamp(src_y_top + ky, 0, (int)frame_height - 1);
          const uint8_t* p_top = frame_data + py_top * frame_stride + px * 4;
          uint8_t r, g, b;
          get_rgb(p_top, r, g, b);
          r_top += r * weight;
          g_top += g * weight;
          ...
          // Bottom half
          int py_bot = std::clamp(src_y_bot + ky, 0, (int)frame_height - 1);
          const uint8_t* p_bot = frame_data + py_bot * frame_stride + px * 4;
          get_rgb(p_bot, r, g, b);
          r_bot += r * weight;
          g_bot += g * weight;
          b_bot += b * weight;
      ...
      r_top /= weight_sum; g_top /= weight_sum; b_top /= weight_sum;
      r_bot /= weight_sum; g_bot /= weight_sum; b_bot /= weight_sum;
...
static std::string render_ascii(
  ...
  for (int ay = 0; ay < h_scaled; ay++) {
    for (int ax = 0; ax < w_scaled; ax++) {
      int src_x = (ax * frame_width) / w_scaled;
      int src_y = (ay * frame_height) / h_scaled;
      ...
      for (int ky = 0; ky < kernel_size; ky++) {
        for (int kx = 0; kx < kernel_size; kx++) {
          int px = std::clamp(src_x + kx, 0, (int)frame_width - 1);
          int py = std::clamp(src_y + ky, 0, (int)frame_height - 1);
...
static void capture_thread(int output_index, int fps) {
  if (output_index >= (int)outputs.size()) return;
  ...
  while (running) {
    ...
    // Wait for frame with condition variable (with timeout)
    {
      std::unique_lock<std::mutex> lock(*output_mutexes[output_index]);
      frame_ready_cvs[output_index]->wait_for(lock, std::chrono::milliseconds(100), [&] {
        return output_ready[output_index] || !running;
      });
    }
    
    if (!output_ready[output_index]) {
      std::cerr << "[CAPTURE] Frame timeout for output " << output_index << "\n";
      continue;
    }
    
    // Frame pacing with adaptive rate detection
    auto frame_end = std::chrono::steady_clock::now();
    auto elapsed_us = std::chrono::duration_cast<std::chrono::microseconds>(
        frame_end - frame_start).count();
    
    // Detect if we're consistently slow
    if (elapsed_us > frame_delay * 1.5) {
      consecutive_slow_frames++;
      if (consecutive_slow_frames > 10) {
        std::cerr << "[PERF] Capture thread consistently slow (avg " 
                  << elapsed_us << "us, target " << frame_delay << "us)\n";
        std::cerr << "[PERF] Consider reducing FPS or resolution\n";
        consecutive_slow_frames = 0;
      }
    } else {
      consecutive_slow_frames = std::max(0, consecutive_slow_frames - 1);
    }
...
static std::vector<uint8_t> compress_frame(const std::vector<uint8_t>& data, 
  ...
  if (total_frames_sent % 100 == 0) {
    ...
    double mbps = ((double)total_bytes_compressed * 8.0 / 1000000.0) * 30; // Assume ~30fps
...
static bool init_microphone_virtual_source(const AudioFormat& fmt) {
  if (!feature_microphone) return true;
...
static void accept_audio_thread(int server_socket) {
  ...
  while (running) {
    ...
    if (client_socket < 0) {
      if (running) std::cerr << "[AUDIO] Accept failed\n";
      continue;
    }
...
static void microphone_thread(int client_socket, std::string session_id) {
  ...
  if (!init_microphone_virtual_source(fmt)) {  // CHANGED
    std::cerr << "[MICROPHONE IN] Failed to init virtual source\n";
    close(client_socket);
    return;
  }
  
  while (running && microphone_virtual_source.running) {  // CHANGED
    if (recv(client_socket, &type, sizeof(type), 0) != sizeof(type)) break;
    ...
    AudioDataHeader header;
    if (recv(client_socket, &header, sizeof(header), 0) != sizeof(header)) break;
    ...
    
    {
      ...
      while (microphone_virtual_source.microphone_queue.size() > 10) {  // CHANGED
...
static void accept_microphone_thread(int server_socket) {
  ...
  while (running) {
    ...
    int client_socket = accept(server_socket, (sockaddr*)&client_addr, &client_len);
    if (client_socket < 0) {
      if (running) std::cerr << "[MICROPHONE IN] Accept failed\n";
      continue;
    }
...
static void update_zoom_smooth_pan(ZoomState& zoom) {
  if (!zoom.smooth_pan || !zoom.enabled) return;
...
static void handle_frame_client(int client_socket, sockaddr_in client_addr) {
  ...
  {   
    ...
    while (running && conn->active && send_queue->running) {
      ...
      // Wait for new frame with condition variable
      {
        std::unique_lock<std::mutex> lock(*output_mutexes[output_index]);
        
        frame_ready_cvs[output_index]->wait_for(lock, std::chrono::milliseconds(100), [&] {
          Capture& cap = output_captures[output_index];
          return (cap.copy_ready && cap.timestamp != last_frame_timestamp) || !running || !conn->active;
        });

        auto now_ = std::chrono::steady_clock::now();
        auto elapsed = std::chrono::duration_cast<std::chrono::microseconds>(
          now_ - last_send).count();
        
        if (elapsed < client_frame_delay) {
          continue;
        }
        ...
        if (!cap.copy_ready || cap.timestamp == last_frame_timestamp) {
          continue;
        }
        
        auto now = std::chrono::steady_clock::now();
        auto age_ms = std::chrono::duration_cast<std::chrono::milliseconds>(
            now - cap.timestamp).count();
        
        if (age_ms > 100) {
          total_frames_dropped++;
          last_frame_timestamp = cap.timestamp;
          continue;
        }
        
        last_frame_timestamp = cap.timestamp;
        local_frame = cap.stable_copy;
        width = cap.width;
        height = cap.height;
        stride = cap.stride;
      }
        ...
        case 0:
          if (config.render_device == 1) {
            rendered = render_braille_cuda_wrapper(
              ...
              mode, keep_aspect_ratio, config.scale_factor, config.detail_level, config.threshold_steps
            );
          }
          else {
...
int main(int argc, char** argv) {
  ...
  signal(SIGINT, [](int) { shutdown_handler(SIGINT); });
  signal(SIGTERM, [](int) { shutdown_handler(SIGTERM); });
  ...
  // Close sockets
  if (frame_server_socket >= 0) close(frame_server_socket);
  if (input_server_socket >= 0) close(input_server_socket);
  if (audio_server_socket >= 0) close(audio_server_socket);
  if (config_server_socket >= 0) close(config_server_socket);
  if (microphone_server_socket >= 0) close(microphone_server_socket);

  // Cleanup resources
  if (feature_video) {
    for (auto& cap : output_captures) {
      if (cap.buffer) {
        munmap(cap.data, cap.size);
        wl_buffer_destroy(cap.buffer);
      }
