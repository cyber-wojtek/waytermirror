    ...
    uint8_t detail_level)
{
    ...
    // Calculate mean colors for light and dark pixel groups
    uint32_t light_r_sum = 0, light_g_sum = 0, light_b_sum = 0, light_count = 0;
    uint32_t dark_r_sum = 0, dark_g_sum = 0, dark_b_sum = 0, dark_count = 0;
    ...
    uint8_t light_r = light_count > 0 ? light_r_sum / light_count : 200;
    uint8_t light_g = light_count > 0 ? light_g_sum / light_count : 200;
    uint8_t light_b = light_count > 0 ? light_b_sum / light_count : 200;

    uint8_t dark_r = dark_count > 0 ? dark_r_sum / dark_count : 55;
    uint8_t dark_g = dark_count > 0 ? dark_g_sum / dark_count : 55;
    uint8_t dark_b = dark_count > 0 ? dark_b_sum / dark_count : 55;

    // The MAJORITY cluster is background, MINORITY is foreground
    // This handles both dark-on-light (text) and light-on-dark (terminal) correctly
    if (light_count >= dark_count)
    {
        // Light is majority = background, dark is minority = foreground
        result.dominant_bg_r = light_r;
        result.dominant_bg_g = light_g;
        result.dominant_bg_b = light_b;
        result.dominant_fg_r = dark_r;
        result.dominant_fg_g = dark_g;
        result.dominant_fg_b = dark_b;
    }
    else
    {
        // Dark is majority = background, light is minority = foreground
        result.dominant_bg_r = dark_r;
        result.dominant_bg_g = dark_g;
        result.dominant_bg_b = dark_b;
        result.dominant_fg_r = light_r;
        result.dominant_fg_g = light_g;
        result.dominant_fg_b = light_b;
    }
    ...
    const RegionalColorAnalysis &regional)
{
    ...
    if (detail_level >= 90)
    {
        ...
        // Use regional context to determine which is foreground vs background
        // Calculate average color of lit vs unlit dots
        ...
        {
            // Calculate distance from each group to regional background
            ...
            // Regional background is the majority color in the area
            // Compare which cell cluster is closer to regional bg (majority)
            double lit_to_reg_bg =
                std::abs(lit_avg_r - regional.dominant_bg_r) +
                std::abs(lit_avg_g - regional.dominant_bg_g) +
                std::abs(lit_avg_b - regional.dominant_bg_b);

            double unlit_to_reg_bg =
                std::abs(unlit_avg_r - regional.dominant_bg_r) +
                std::abs(unlit_avg_g - regional.dominant_bg_g) +
                std::abs(unlit_avg_b - regional.dominant_bg_b);

            // If lit dots are closer to regional background, invert
            // (lit should be foreground/minority, not background)
